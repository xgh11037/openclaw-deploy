...
function walkFiles(dir, out = []) {
  ...
  for (const name of readdirSync(abs)) {
    ...
    const st = statSync(p);
    if (st.isDirectory()) {
      if (['node_modules', 'dist', 'target', '.git', 'github-artifacts', 'release', 'resources', 'bundled-node-modules', 'bundled-extensions'].includes(name)) continue;
...
check('relay api base env default', app.includes('VITE_YUNRUI_RELAY_API_BASE_URL'));
check('relay provider option visible', app.includes('label: "云睿中转站"') && app.includes('setProvider("relay")'));
check('relay foreign model options visible', app.includes('GPT-4o') && app.includes('Claude 3.5 Sonnet') && app.includes('Gemini 2.0 Flash'));
check('relay base url guidance visible', app.includes('填云睿中转站 API 地址，例如 https://.../v1'));
